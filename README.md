...
A clone of Omarchy's built-in agents usage panel, with a Grok collector so
SuperGrok weekly usage shows up next to Claude Code, Codex, and Fireworks.

Install it on another Omarchy machine with:

```bash
omarchy plugin add https://github.com/tony-roslund/omarchy-agents-plugin.git --enable
```

That replaces the stock `omarchy.agents` widget with this clone (`tony.agents`).
Update later with:

```bash
omarchy plugin update tony.agents --yes
```

Remove it with `omarchy plugin remove tony.agents --yes` to get the stock
panel back.
...
| `grok` | SuperGrok weekly pool and Grok Build product usage from cli-chat-proxy `/v1/billing` | `$GROK_HOME/sessions` `turn_completed` ledgers (plus pi/omp and opencode sessions on an xAI provider) |
| `fireworks` | Estimated prepaid balance: configured funding minus rated account costs | Fireworks billing API, grouped by day and model for the last 30 days |

This clone ships `omarchy-agent-usage-grok` because the packaged updater only globs `$OMARCHY_PATH/bin`. Refresh runs `update` in this directory, which writes `grok.json` next to the other records. A non-default Grok home is honored via `GROK_HOME`. Grok limits need `grok login`; without credentials the panel says so and falls back to local stats.
