...
// The display side of agent usage. Packaged collectors write one JSON
// record per agent into the usage directory; this clone also runs its
// Grok collector so the panel gains a Grok tab. This file discovers those
// records, watches them for changes, and optionally merges snapshots
// synced from other machines.
Item {
  id: root
  visible: false
  ...
  function updateCommand(kind, agentIds) {
    var command = [root.pluginDir + "/update"]
